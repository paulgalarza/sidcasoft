...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-06-2015 a las 06:50:03
...
`idProyecto` int(10) NOT NULL,
  ...
  `usuarioRAP` int(10) unsigned DEFAULT NULL,
  `usuarioRCP` int(10) unsigned DEFAULT NULL,
  `usuarioAnalista` int(10) unsigned DEFAULT NULL,
  `usuarioArquitecto` int(10) unsigned DEFAULT NULL,
  `usuarioDesarrollador` int(10) unsigned DEFAULT NULL,
  `usuarioTester` int(10) unsigned DEFAULT NULL,
...
--
DELIMITER //
CREATE TRIGGER `TR_AsignaProyecto` AFTER INSERT ON `proyecto`
 FOR EACH ROW begin
...
DELIMITER ;
DELIMITER //
CREATE TRIGGER `TR_AsignaProyecto_Actualiza` AFTER UPDATE ON `proyecto`
 FOR EACH ROW begin
...
DELIMITER ;
DELIMITER //
CREATE TRIGGER `TR_AsignaProyecto_Elimina` AFTER DELETE ON `proyecto`
 FOR EACH ROW begin
    Update usuarios set ProyectoAsignado = NULL where idUsuario = OLD.usuarioRAP;
    Update usuarios set ProyectoAsignado = NULL where idUsuario = OLD.usuarioRCP;
    Update usuarios set ProyectoAsignado = NULL where idUsuario = OLD.usuarioAnalista;
